...
class StatusDropdown extends StatefulWidget {
  ...
  const StatusDropdown({super.key, required this.report, required this.onStatusChanged});
...
class _StatusDropdown extends State<StatusDropdown> {
  ...
  Widget build(BuildContext context) {
    Color statusColour = _getStatusColour(selectedStatus); // Function to get color based on status
    ...
    return Status.values.map<Widget>((Status status) {
      return Center(child: Text(
        ...
        style: const TextStyle(color: Colors.white), // Set desired color for unfocused item
      ));
    }).toList();
  },
          elevation: 8,
          ...
          items: Status.values.map((Status status) {
            return DropdownMenuItem<Status>(
              value: status,
              child: Text(status.name, style: TextStyle(color: _getStatusColour(status)),),
            );
          }).toList(),
          onChanged: (Status? newValue) {
            setState(() {
              selectedStatus = newValue!;
              widget.onStatusChanged(newValue); // Notify the parent widget about status change
            });
          },
          // Customize dropdown appearance
          dropdownColor: const Color(0xcc000000), // Dropdown will have the same color
          style: TextStyle(color: _getStatusColour(selectedStatus)), // Customize text color
        ),
      ),
    );
  }
